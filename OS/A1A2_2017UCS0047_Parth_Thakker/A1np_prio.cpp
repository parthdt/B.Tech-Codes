...
bool comparefn(vector <int> a, vector <int> b){     //this function shall be used in STL sort(),
    if(a[0]==b[0])return a[3]<b[3];                 //If arrival time is same, sort acc. to priority, else sort acc. to arrival time
    return a[0]<b[0];
}
...
int main(){
    ...
    vector<vector<int>> p;
    p.resize(n);
    for(int j=0;j<n;j++)p[j].resize(4,0);
    int i;
    for(i=0;i<n;i++){
        ip>>p[i][0]>>p[i][1]>>p[i][3];
        p[i][2]=i+1;
    }
    ip.close();
    ...
    for(i=0;i<n;i++){
        ...
        wt[i]=rt[i]=ct-p[i][0];
        ct+=p[i][1];
        // for(int j=i;j<n;j++)p[j][0]=max(0,(p[j][0]-ct));        //update arrival times so as to take cae of processes that came in between the process that was running
        sort(p.begin()+i+1,p.end(),comparefn);
